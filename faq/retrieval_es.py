...
class SearchData(object):
    ...
    def search_es(self, query_word_list, ES_limit_num):
        """
        Author: xiaoyichao
        param {type}
        Description: 使用ES召回
        ...
        retrieve_data = es_faq.search_data(
            ...
            query_word_list=query_word_list,
            limit_num=ES_limit_num,
        )
    ...
    def search_faiss(self, question, num=5):
        ...
        faiss_index_path = os.path.join(dir_name, "../es/search_model/faiss.index")
    ...
    def merge_op(
        ...
        if use_faiss == 1 and use_annoy == 0:
            print("use_faiss")
            mayey_search_questions = self.search_faiss(question, num=engine_limit_num)
        elif use_faiss == 0 and use_annoy == 1:
            print("use_annoy")
            mayey_search_questions = self.search_annoy(question, num=engine_limit_num)
        elif use_faiss == 1 and use_annoy == 1:
            print("use_annoy and use_faiss ")
            mayey_search_questions_faiss = self.search_faiss(
                question, num=engine_limit_num
            )
        ...
        for sim_question in mayey_search_questions:
            if sim_question not in set(maybe_original_questions):
                ...
                retrieve_data = es_faq.search4search_engine(
                    index_name, question=sim_question
                )
                retrieve_results = retrieve_data["hits"]
                max_result_len = retrieve_results["total"]["value"]
                # max_score = retrieve_results["max_score"]
    ...
    def search_merge(
        ...
        maybe_original_questions, maybe_process_questions, maybe_answers, q_ids, specific_q_ids = self.search_es(
            query_word_list=query_word_list, ES_limit_num=ES_limit_num
        )
        print("maybe_original_questions_es", maybe_original_questions)
