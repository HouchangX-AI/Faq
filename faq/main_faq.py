...
use_other_when_es_none = int(
    faq_config["AlgorithmConfiguration"]["use_other_when_es_none"]
)
...
async def myfaq(request):
    ...
    # 给ES使用的结巴分词
    process_query = stopwords4BEFAQ_before_search.seg_sentence(sentence=orgin_query)
    ...
    maybe_original_questions, maybe_process_questions, maybe_answers, retrieval_q_ids, specific_q_ids = search_data.search_merge(
        question=orgin_query,
        query_word_list=query_word_list,
        use_faiss=use_faiss,
        use_annoy=use_annoy,
        engine_limit_num=engine_num,
        ES_limit_num=ES_num,
        use_other_when_es_none=use_other_when_es_none,
    )
    ...
    if len(retrieval_q_ids) > 0:  # ES（或faiss 或 annoy ）中检索到了数据
        ...
        consin_sim = match_ing.cosine_sim(
            orgin_query=orgin_query, retrieval_questions=maybe_original_questions
        )
        ...
        re_rank_sim = rerank.linear_model(
            consin_sim=consin_sim,
            jaccard_sim=jaccard_sim,
            consine_weight=consine_weight,
        ...
        return_data = final_data.get_qa(
            ...
            maybe_answers,
            re_rank_sim=re_rank_sim,
            get_num=get_num,
            retrieval_q_ids=retrieval_q_ids,
            specific_q_ids=specific_q_ids,
        )
